...
export class FileTooLargeError extends Error {
  constructor(
    ...
    super(`File too large: ${actualMB}MB exceeds maximum ${maxMB}MB for ${mimeType}`)
...
export async function truncateFile(
  ...
  const { maxPages = DEFAULT_MAX_PAGES, maxSizeBytes = DEFAULT_MAX_SIZE_BYTES } = options
  ...
  if (mimeType === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
    return truncateDocxFile(fileBytes, maxPages, maxSizeBytes, options.paragraphsPerPage)
  }

  if (BINARY_DOCUMENT_MIME_TYPES.includes(mimeType as typeof BINARY_DOCUMENT_MIME_TYPES[number])) {
    return validateFileSize(fileBytes, mimeType, maxSizeBytes)
  }
...
function truncateTextFile(
  ...
  const finalBytes = new Uint8Array(truncatedBytes.length + truncationBytes.length)
...
async function truncatePptxToSlides(
  ...
  if (presRelsXml) {
    const parsed = await parseStringPromise(presRelsXml, { explicitArray: true })
  ...
  if (contentTypesXml) {
    const parsed = await parseStringPromise(contentTypesXml, { explicitArray: true })
...
async function cleanupOrphanedPptxMedia(zip: JSZip, builder: Builder): Promise<void> {
...
async function truncateDocxFile(
  ...
  const actualParagraphsPerPage = paragraphsPerPage ?? DEFAULT_PARAGRAPHS_PER_PAGE
  ...
  if (totalParagraphs <= initialMaxParagraphs && fileBytes.length <= maxSizeBytes) {
    return {
      bytes: fileBytes,
      wasTruncated: false,
      originalSize,
...
async function truncateDocxToParagraphs(
  ...
  if (docRelsXml) {
    const relsParsed = await parseStringPromise(docRelsXml, { explicitArray: true })
  ...
  if (contentTypesXml) {
    const ctParsed = await parseStringPromise(contentTypesXml, { explicitArray: true })
...
function collectRelationshipIds(obj: any, ids = new Set<string>()): Set<string> {
...
function isRequiredDocxRelationship(type: string): boolean {
  ...
  return requiredTypes.some((t) => type.toLowerCase().includes(t.toLowerCase()))
}
...
async function cleanupFootnotes(
  ...
  const usedFootnoteIds = collectElementIds(docParsed, 'w:footnoteReference', 'w:id')

  const parsed = await parseStringPromise(footnotesXml, { explicitArray: true })
...
async function cleanupEndnotes(
  ...
  const usedEndnoteIds = collectElementIds(docParsed, 'w:endnoteReference', 'w:id')
