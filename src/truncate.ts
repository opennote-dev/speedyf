...
import { DEFAULT_MAX_FILE_SIZE_BYTES, DEFAULT_MAX_PAGES, GENERIC_MIME_TYPES } from "./config.js";
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      if (typeof file === "string") {
        return c.json(
          { error: "File must be a binary file, not a string" },
          400,
        );
      }
      ...
      paragraphsPerPage = body["paragraphsPerPage"]
        ? parseInt(body["paragraphsPerPage"] as string)
        : undefined;
    } else {
      const body = await c.req.json();
      const {
        url,
        maxPages: mp = DEFAULT_MAX_PAGES,
        maxSizeBytes: ms = DEFAULT_MAX_FILE_SIZE_BYTES,
...
const truncationGet = async (c: Context) => {
  try {
    ...
    const maxPages = parseInt(
      c.req.query("maxPages") || String(DEFAULT_MAX_PAGES),
    );
    const maxSizeBytes = parseInt(
      c.req.query("maxSizeBytes") || String(DEFAULT_MAX_FILE_SIZE_BYTES),
    );
    const paragraphsPerPage = c.req.query("paragraphsPerPage")
      ? parseInt(c.req.query("paragraphsPerPage")!)
      : undefined;
